...
1. Indexes
  - Add various indexes to improve query performance across multiple tables
  - Focus on commonly queried columns and relationships
  - Add GIN indexes for JSON fields to enable efficient querying of nested data

This migration adds indexes to improve database performance without modifying schema structure.
*/
...
-- Add index for session players by session (for session player listing)
