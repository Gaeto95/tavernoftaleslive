...
1. New Functions
   - `cleanup_old_chat_messages`: Removes chat messages older than specified days
   - `cleanup_orphaned_sessions`: Marks sessions with no players as inactive
   - `cleanup_user_sessions`: Removes a specific user from all sessions
   - `has_active_players`: Checks if a session has any players
   - `run_session_cleanup`: Runs all cleanup operations

2. Security
   - Added RLS policy for session_cleanup_queue
*/
